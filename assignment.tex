...
When we have a 2D point cloud with gaps or sparse regions and we need a way to fill those holes with a good quality triangle mesh. The approach we took is to first build a Delaunay triangulation of the point cloud using the Bowyer-Watson algorithm then identify triangles that are too large or too thin, these are the ones covering the holes and sparse areas and use Ruppert's refinement algorithm to insert new points into those bad triangles until the mesh is filled in properly.
...
The application lets us generate a random 2D point cloud then it triangulates the points and highlights where the holes and bad triangles are. We can then refine the mesh step by step or all at once and watch as the holes get filled in with new points and better shaped triangles.
...
If we have three points that are not all sitting on the same line then there is always going to be exactly one circle we can draw that passes through all three of them, that circle is what we call the circumcircle and the center of it is the circumcenter. We can find the circumcenter by drawing the perpendicular bisector of two of the triangle's edges and seeing where they cross each other, the crossing point is the circumcenter. We implemented this in \texttt{geometry.ts} using line equation helpers:
...
Not every triangle that comes out of the Delaunay triangulation is going to look good. As a matter of fact, some will end up really large, some end up really thin and pointy and we need a way to flag these so we know which ones need to be fixed. A triangle gets flagged as ``bad'' if its area is too large, the default threshold we set is 8000 but the user can adjust it with a slider, or if its smallest angle is too small which defaults to 20\textdegree and is also adjustable. The code for both checks:
...
At this stage we know which triangles are not up to par with our definition of good triangle and we need to refine them, this is where Ruppert's refinement comes in. The idea is we pick a bad triangle, find a good spot inside it to put a new point and add that point to the point set then re-run Bowyer-Watson so the whole mesh gets re-triangulated with the new point included. The full implementation is in \texttt{refinement.ts}:
...
For the points we also use color to differentiate between the original random points and the ones that were inserted during refinement. The original points show up as dark circles and the refinement inserted points show up as purple circles. We thought this distinction was important because it lets us see how much work the refinement algorithm actually did compared to the starting point set. There is also a statistics bar at the bottom of the page that shows total points, how many were added by refinement, triangle count, bad triangle count, and the average minimum angle across the whole mesh.
