...
So now we know which triangles are bad and we need to fix them, this is where Ruppert's refinement comes in. The idea is we pick a bad triangle, find a good spot inside it to put a new point, add that point to the point set, and then re-run Bowyer-Watson so the whole mesh gets re-triangulated with the new point included. The full implementation is in \texttt{refinement.ts}:
